...
import type { AppConfigProps, ColorScheme, LayoutConfig } from "~/types";
...
const AppConfig = (props: AppConfigProps) => {
  ...
  useEffect(() => {
    if (isSlim() || isSlimPlus() || isHorizontal()) {
      setLayoutState((prevState: any) => ({ ...prevState, resetMenu: true }));
    }
      ...
      >
          ...
          {componentThemes.map((theme, i) => {
            return (
              <div key={i} className="w-3">
                <button
                  type="button"
                  className="p-link w-2rem h-2rem border-circle align-items-center justify-content-center flex flex-shrink-0 cursor-pointer"
        ...
        {!props.minimal && (
          <>
            <h5>Menu Type</h5>
            <div className="row-gap-3 flex flex-wrap">
              <div className="align-items-center flex w-6 gap-2">
                <RadioButton
                  name="menuMode"
                  value={"static"}
